...
proof:element kind=label field=component width=16
proof:element kind=badge field=required width=10
...
```proof:tree kind=org
root: URI examples by specificity
...
```proof:tree kind=taxonomy source=md://src/data/label-matching.md name=strategy parent=priority
```
