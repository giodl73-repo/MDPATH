...
pub fn apply_table_subselectors(
    ...
    Ok(TableSelection { content, row_idx, col_idx })
}

fn find_row(table: &ParsedTable, value: &SelectorValue) -> Result<usize, MdPathError> {
    match value {
        SelectorValue::Index(n) => {
            if *n < table.rows.len() { Ok(*n) }
            else { Err(MdPathError::SubKeyNotFound(format!("row index {} out of range", n))) }
        }
...
fn find_col(table: &ParsedTable, value: &SelectorValue) -> Result<usize, MdPathError> {
    match value {
        SelectorValue::Index(n) => {
            if *n < table.headers.len() { Ok(*n) }
            else { Err(MdPathError::SubKeyNotFound(format!("column index {} out of range", n))) }
        }
        SelectorValue::Named(name) => {
            ...
            for (i, header) in table.headers.iter().enumerate() {
                ...
                if norm_h == norm_name { return Ok(i); }
                if norm_h.starts_with(&norm_name) && found.is_none() { found = Some(i); }
            }
...
fn build_table_content(table: &ParsedTable, row_idx: Option<usize>, col_idx: Option<usize>) -> String {
    match (row_idx, col_idx) {
        ...
        (Some(r), None) => {
            ...
            let header = table.headers.iter().map(|h| h.trim()).collect::<Vec<_>>().join(" | ");
            let row = table.rows.get(r)
        ...
        (None, Some(c)) => {
            // Whole column
            let mut lines = vec![table.headers.get(c).map(|h| h.trim().to_string()).unwrap_or_default()];
        ...
        match sub.key.as_str() {
            ...
            "row" => {
                ...
                if let SelectorValue::Index(n) = &sub.value {
                    return block.content.get(*n)
                        .map(|s| s.clone())
                        .ok_or_else(|| MdPathError::SubKeyNotFound(
                            format!("line {} out of range (figure has {} lines)", n, block.content.len())
                        ));
                }
            }
            other => {
...
fn find_box_in_figure(lines: &[String], value: &SelectorValue) -> Result<String, MdPathError> {
    ...
    match value {
        ...
        SelectorValue::Named(name) => {
            ...
            if count > 0 { return Err(MdPathError::LabelAmbiguous(name.clone(), count + 1)); }
...
fn is_top_border(trimmed: &str) -> bool {
    ...
    && trimmed.chars().filter(|c| matches!(c, '┌'|'┐'|'+'|'╔'|'╗')).count() >= 1
}

fn is_any_border(trimmed: &str) -> bool {
    matches!(trimmed.chars().next(),
...
fn extract_box_label(box_content: &str) -> Option<String> {
    ...
    for line in box_content.lines().skip(1) {
        ...
        if !inner.is_empty() {
            return Some(inner.split_whitespace().take(3).collect::<Vec<_>>().join(" "));
        }
    }
    None
}
...
pub fn apply_table_query(
    ...
    let col_indices: Option<Vec<usize>> = select_cols.map(|cols| {
        cols.iter().filter_map(|c| {
            table.headers.iter().position(|h| h.trim() == c.as_str())
        }).collect()
    });
    ...
    let header = match &col_indices {
        None => table.headers.iter().map(|h| h.trim()).collect::<Vec<_>>().join(" | "),
        Some(idxs) => idxs.iter()
...
fn row_matches_filter(row: &[String], headers: &[String], filter: &str) -> bool {
    ...
    if parts.len() < 3 { return true; } // unparseable → pass all
...
mod tests {
    ...
    fn make_table() -> ParsedTable {
        ParsedTable {
            ...
            line_start: 1, line_end: 5, heading_idx: 0,
        }
    }
    ...
    fn cell_row_and_col() {
        let table = make_table();
        let subs = vec![
            SubSelector { key: "row".into(), value: SelectorValue::Named("Typing".into()) },
            SubSelector { key: "col".into(), value: SelectorValue::Named("Value".into()) },
        ];
